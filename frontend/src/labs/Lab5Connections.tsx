...
import { describeParsed, parseConnectionString } from '../lib/connectionString'
import type {
  ConnectionMeta,
  DeployResult,
  DriverInfo,
...
const CS_EXAMPLES = [
  'postgresql://readonly_user:••••@db.example.com:5432/analytics?sslmode=require',
  'mysql://readonly_user:••••@10.0.4.12:3306/operations',
  'Server=tcp:sql.example.com,1433;Database=analytics;User Id=ro;Password=••••;Encrypt=true',
  'host=db.example.com port=5432 dbname=analytics user=ro password=••••',
]

/**
 * Paste-a-DSN entry. Parses on every keystroke so the reader can see exactly
 * which field each part of their string maps to *before* anything is applied —
 * a connection string is dense, and silently filling seven inputs from it is
 * how you end up connecting to the wrong database.
 */
function ConnStringEntry({
  value, error, onChange, onApply,
}: {
  value: string
  error?: string
  ...
  const rows: [string, string | undefined][] = parsed
    ? [
        ['Engine', parsed.driver],
        ['Host', parsed.host],
        ['Port', parsed.port ? String(parsed.port) : undefined],
        ['Database', parsed.database],
        ['Username', parsed.username],
        ['Password', parsed.password ? '•'.repeat(Math.min(12, parsed.password.length)) : undefined],
        ['SSL mode', parsed.sslMode],
      ]
    : []
  ...
  return (
    <div className="cs-entry">
      <label className="form-field">
        <span>Connection string <Lock width={11} height={11} /></span>
        <textarea
          className={`field cs-input ${error ? 'invalid' : ''}`}
          rows={3}
          value={value}
      ...
      {parsed ? (
        <div className="cs-preview">
          <div className="cs-preview-head">Extracted</div>
          <dl className="cs-grid">
            {rows.map(([label, val]) => (
              <div key={label} className={`cs-row ${val ? '' : 'missing'}`}>
                <dt>{label}</dt>
                <dd>{val ?? '— not in the string'}</dd>
              </div>
            ))}
          </dl>
          {parsed.warnings.map((w) => (
            <p key={w} className="cs-warn">{w}</p>
          ))}
        </div>
      ) : (
        <div className="cs-preview cs-preview-empty">
          <div className="cs-preview-head">Recognised formats</div>
          <ul className="cs-examples">
            {CS_EXAMPLES.map((ex) => <li key={ex}><code>{ex}</code></li>)}
          </ul>
        </div>
      )}
    </div>
...
export default function Lab5Connections() {
  ...
  const [entry, setEntry] = useState<'fields' | 'string'>('fields')
  ...
  /**
   * Push a parsed connection string into the same fields the form collects, so
   * everything downstream — validation, save, encryption — is unchanged.
   * The string itself is cleared afterwards rather than left sitting in the DOM
   * with a plaintext password in it.
   */
  function applyConnString() {
    const parsed = parseConnectionString(connString)
    if (!parsed) {
      setFieldErrors({ connString: "That doesn't look like a connection string yet." })
      return
    }
    if (parsed.driver && parsed.driver !== driver) {
      const target = drivers.find((d) => d.id === parsed.driver)
      if (target?.available) setDriver(target.id)
    }
    if (parsed.host) setHost(parsed.host)
    if (parsed.port) setPort(parsed.port)
    else if (driverMeta && !port) setPort(driverMeta.default_port)
    if (parsed.database) setDatabase(parsed.database)
    if (parsed.username) setUsername(parsed.username)
    if (parsed.password) setPassword(parsed.password)
    if (parsed.sslMode) setSslMode(parsed.sslMode)

    setApplied(describeParsed(parsed))
    setConnString('')
    setFieldErrors({})
    setEntry('fields') // land on the filled form so nothing is applied unseen
  }
        ...
        {step === 2 && (
          <div className="wstep">
            <div className="wstep-head">
              ...
              <span className="wstep-title">Connection details — {driverMeta?.label}</span>
              <div className="wstep-aux entry-toggle" role="tablist" aria-label="How to enter connection details">
                <button
                  role="tab"
                  aria-selected={entry === 'fields'}
                  className={`entry-tab ${entry === 'fields' ? 'on' : ''}`}
                  onClick={() => setEntry('fields')}
                >
                  Fields
                </button>
                <button
                  role="tab"
                  aria-selected={entry === 'string'}
                  className={`entry-tab ${entry === 'string' ? 'on' : ''}`}
                  onClick={() => { setEntry('string'); setApplied(null) }}
                >
                  Connection string
                </button>
              </div>
            ...
            {entry === 'string' ? (
              <ConnStringEntry
                value={connString}
                error={fieldErrors.connString}
                onChange={(v) => { setConnString(v); if (fieldErrors.connString) setFieldErrors({}) }}
                onApply={applyConnString}
              />
            ) : (
            ...
            {applied && entry === 'fields' && (
              <p className="cs-applied">
                <Check width={13} height={13} /> Filled from your connection string — <code>{applied}</code>.
                Check it over before continuing.
              </p>
            )}
            ...
            <div className="wizard-actions">
              ...
              {entry === 'string' ? (
                <button className="btn btn-primary" onClick={applyConnString} disabled={!connString.trim()}>
                  Extract details <ArrowRight width={15} height={15} />
                </button>
              ) : (
