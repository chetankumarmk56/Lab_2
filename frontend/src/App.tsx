...
import Lab6PolicyRag from './labs/Lab6PolicyRag'
import Lab7DeepResearch from './labs/Lab7DeepResearch'
import Lab8Evals from './labs/Lab8Evals'
import Guide from './guide/Guide' // guide: self-contained in src/guide/ — delete the folder and this line to remove
import { useTheme } from './lib/useTheme'
import { ArrowRight, Bookmark, Moon, Sun } from './components/icons'
...
const LABS: LabMeta[] = [
  ...
  {
    path: '/lab6', num: 6, title: 'Policy Q&A (RAG)', sector: 'Public Sector', ready: true,
    blurb: 'Ask policy questions and watch every RAG stage — chunks, retrieval scores, fusion, prompt, grounded answer.',
  },
  {
    path: '/lab7', num: 7, title: 'Deep Research', sector: 'Orchestration', ready: true,
    blurb: 'A planner decomposes your question; you approve; parallel web researchers, a synthesizer, and an adversarial critic do the rest — live.',
  },
  {
    path: '/lab8', num: 8, title: 'Eval Harness', sector: 'Quality', ready: true,
    blurb: 'Golden suites grade Labs 2, 6 and 7 — SQL execution-match, retrieval hit@k, grounded answers, SLOs — with cost, history, and regression diffs.',
  },
]
    ...
    <div className="home">
      <header className="home-hero">
        ...
        <p className="subtitle">
          Eight scenario-driven agents for manufacturing and public-sector teams — each one a
    ...
    <div className="app">
      <aside className="sidebar">
        ...
        <nav>
          ...
          {/* guide: remove this NavLink along with src/guide/ */}
          <NavLink to="/guide" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
            <span className="nav-num"><Bookmark width={13} height={13} /></span>
            <span className="nav-label">Field Guide</span>
          </NavLink>
          {LABS.map((l) => (
            <NavLink
              key={l.path}
      ...
      <main className="content">
        <Routes>
          ...
          <Route path="/guide" element={<Guide />} />{/* guide: remove with src/guide/ */}
          ...
          <Route path="/lab6" element={<Lab6PolicyRag />} />
          <Route path="/lab7" element={<Lab7DeepResearch />} />
          <Route path="/lab8" element={<Lab8Evals />} />
        </Routes>
      </main>
    </div>
  )
