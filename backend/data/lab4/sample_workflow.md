...
## People involved
- **Counter staff** run the renewal at the counter.
- The **Title Corrections desk** handles vehicle-record mismatches.
- A **shift supervisor** must authorize any fee override or manual exception.
...
3. Confirm the vehicle details on screen (make, model, year, owner name) match the citizen's documents. If they do not match, stop and refer the citizen to the Title Corrections desk — do not edit the record yourself.
...
6. Select the renewal term: 1 year or 2 years. Read the displayed fee total aloud to the citizen and confirm they accept it. Any fee adjustment or waiver requires shift-supervisor authorization.
7. Collect payment. For card payments, insert/tap on the counter terminal; for cash, enter the amount tendered and give change from the drawer. If the card is declined, offer an alternate payment method — never re-run a declined card more than twice.
...
- If the portal times out mid-transaction, the payment is NOT captured — re-check the citizen's account before collecting payment again to avoid a double charge.

## Terms
- **VIN** — Vehicle Identification Number.
- **Hold** — a compliance block (emissions, insurance, citations) that stops a renewal.
